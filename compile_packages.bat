...
if exist %WORK_FOLDER%pom.xml (
  call %MVN_BIN% -q clean package %MAVEN_PACKAGE_ARGUMENTS% -Dmaven.test.skip=true -f %WORK_FOLDER%pom.xml
  if exist %WORK_FOLDER%target\*.war copy /y %WORK_FOLDER%target\*.war %WORK_FOLDER%release >nul
  if exist %WORK_FOLDER%\target\ rd /s /q %WORK_FOLDER%\target\ >nul
)
