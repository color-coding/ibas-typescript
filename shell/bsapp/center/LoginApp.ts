        ...
        export class LoginApp extends ibas.Application<ILoginView> {
            ...
            run(): void {
                ...
                if (ibas.strings.isEmpty(userToken)) {
                    ...
                    userToken = sessionStorage.getItem(btoa(ibas.strings.toHex(ibas.CONFIG_ITEM_USER_TOKEN)));
                    if (!ibas.objects.isNull(userToken)) {
                        try {
            ...
            private onConnectCompleted(opRslt: ibas.IOperationResult<bo.IUser>): void {
                try {
                    ...
                    if (ibas.config.get(CONFIG_ITEM_ENABLE_REFRESH_LOGOUT) === false) {
                        ...
                        sessionStorage.setItem(btoa(ibas.strings.toHex(ibas.CONFIG_ITEM_USER_TOKEN)), ibas.strings.toHex(builder.toString()));
                    }
                    // 注册运行变量
