...
export class CustomerListApp extends ibas.BOListApplication<ICustomerListView, bo.Customer> {
    ...
    /** 应用标识 */
    static APPLICATION_ID: string = "ee052eff-16dd-462b-ae52-1fc19b46c387";
    /** 应用名称 */
    static APPLICATION_NAME: string = "trainingtesting_app_customer_list";
    /** 业务对象编码 */
    static BUSINESS_OBJECT_CODE: string = bo.Customer.BUSINESS_OBJECT_CODE;
    /** 构造函数 */
    constructor() {
        super();
        this.id = CustomerListApp.APPLICATION_ID;
        this.name = CustomerListApp.APPLICATION_NAME;
        this.boCode = CustomerListApp.BUSINESS_OBJECT_CODE;
    ...
    protected deleteData(data: bo.Customer): void {
        ...
        let beDeleteds: ibas.ArrayList<bo.Customer> = new ibas.ArrayList<bo.Customer>();
        if (data instanceof Array) {
            for (let item of data) {
                if (ibas.objects.instanceOf(item, bo.Customer)) {
                    item.delete();
                    beDeleteds.add(item);
                }
            }
        ...
        this.messages({
            ...
            onCompleted(action: ibas.emMessageAction): void {
                if (action === ibas.emMessageAction.YES) {
                    try {
                        let boRepository: BORepositoryTrainingTesting = new BORepositoryTrainingTesting();
                        let saveMethod: Function = function (beSaved: bo.Customer): void {
