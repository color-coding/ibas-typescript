...
export class MaterialListApp extends ibas.BOListApplication<IMaterialListView, bo.Material> {
    ...
    /** 应用标识 */
    static APPLICATION_ID: string = "a58c591a-322a-46a7-9c2f-873fba0dd533";
    /** 应用名称 */
    ...
    /** 删除数据，参数：目标数据集合 */
    protected deleteData(data: bo.Material): void {
        // 检查目标数据
        if (ibas.objects.isNull(data)){
            this.messages(ibas.emMessageType.WARNING, ibas.i18n.prop("sys_shell_ui_please_chooose_data",
        ...
        if (data instanceof Array ) {
            for (let item of data) {
                if (item instanceof bo.Material) {
                    item.delete();
                    beDeleteds.add(item);
                }
            }
            ...
            onCompleted(action: ibas.emMessageAction): void {
                if (action === ibas.emMessageAction.YES) {
                    try {
                        ...
                        let method:Function = function(beSaved: bo.Material):void {
