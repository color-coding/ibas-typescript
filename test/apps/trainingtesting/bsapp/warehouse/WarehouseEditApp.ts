...
export class WarehouseEditApp extends ibas.BOEditApplication<IWarehouseEditView, bo.Warehouse> {
    ...
    /** 应用标识 */
    static APPLICATION_ID: string = "615a1292-52ff-4194-b6c9-43ab0780ba3f";
    /** 应用名称 */
    static APPLICATION_NAME: string = "trainingtesting_app_warehouse_edit";
    /** 业务对象编码 */
    ...
    protected viewShowed(): void {
        ...
        if (ibas.objects.isNull(this.editData)) {
            // 创建编辑对象实例
            this.editData = new bo.Warehouse();
            ...
            if (!ibas.objects.isNull(criteria) && criteria.conditions.length > 0) {
                ...
                boRepository.fetchWarehouse({
                    ...
                    onCompleted(opRslt: ibas.IOperationResult<bo.Warehouse>): void {
                        let data: bo.Warehouse;
                        if (opRslt.resultCode === 0) {
                            data = opRslt.resultObjects.firstOrDefault();
                        }
                        if (ibas.objects.instanceOf(data, bo.Warehouse)) {
                            // 查询到了有效数据
                            that.editData = data;
                            that.show();
                        } else {
    ...
    protected createData(clone: boolean): void {
        ...
        let createData: Function = function (): void {
            if (clone) {
                ...
                // 新建对象
                that.editData = new bo.Warehouse();
