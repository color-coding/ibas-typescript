    ...
    export namespace test {
        export class TestBusinessObject extends ibas.TestCase {
            testALL(): void {
                ...
                // 测试状态
                ibas.asserts.equals("bo status isNew", order.isNew, true);
                ibas.asserts.equals("bo status isDirty", order.isDirty, true);
                ibas.asserts.equals("bo status isDeleted", order.isDeleted, false);
                order.markOld(true);
                ibas.asserts.equals("bo status isNew", order.isNew, false);
                ibas.asserts.equals("bo status isDirty", order.isDirty, false);
                ibas.asserts.equals("bo status isDeleted", order.isDeleted, false);
                for (let item of order.salesOrderItems) {
                    ibas.asserts.equals("bo status isNew", item.isNew, false);
                    ibas.asserts.equals("bo status isDirty", item.isDirty, false);
                    ibas.asserts.equals("bo status isDeleted", item.isDeleted, false);
                }
                ibas.logger.log(ibas.emMessageLevel.INFO, "test: {0} {1}", "order", order.getProperties(false).size);
                ...
                ibas.asserts.equals("clone bo instance is not change.", order === cOrder, false);
                // 测试查询
                ...
                ibas.asserts.equals("clone bo instance is not change.", criteria === cCriteria, false);
                ...
                boRepository.saveSalesOrder({
                    ...
                    onCompleted(opRslt: ibas.IOperationResult<SalesOrder>): void {
                        ...
                        if (newOrder) {
                            ibas.asserts.equals("order document status wrong.", order.documentStatus, newOrder.documentStatus);
                        }
                    }
                });
                // 测试离线仓库
                ...
                item = order.salesOrderItems.create();
                // afterAdd赋初值
                ibas.asserts.equals("item.lineStatus is default value.",
                    item.lineStatus === ibas.emDocumentStatus.RELEASED, true);
                // 父项更改修改子项
                order.documentStatus = ibas.emDocumentStatus.CLOSED;
                ibas.asserts.equals("item.lineStatus is not changed to CLOSED.",
                    item.lineStatus === ibas.emDocumentStatus.CLOSED, true);
                // 子项更改修改父项
                item.lineTotal = 100;
                ibas.asserts.equals("order.documentTotal does not equal the total number of item.lineTotal.",
                    order.documentTotal === 100, true);
                // 移除子项时移除子项监听事件
                let listenerCount: number = (<any>item).listeners().length;
                order.salesOrderItems.remove(item);
                ibas.asserts.equals("item is not remove the listener after it removed from the collection",
